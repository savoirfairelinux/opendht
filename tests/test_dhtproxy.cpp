...
namespace {
// The result is produced on the DHT thread, which may be blocked in a listen
// callback waiting for the same mutex: never hold it while waiting.
dht::PushNotificationResult
sendPush(dht::DhtRunner& node, const std::map<std::string, std::string>& data, std::unique_lock<std::mutex>& lk)
{
    lk.unlock();
    auto ret = node.pushNotificationReceived(data).get();
    lk.lock();
    return ret;
}
} // namespace
...
DhtProxyTester::testPushListenValueEdit()
{
    ...
    push_data["t"] = std::to_string(
        std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch())
            .count());
    sendPush(nodeClient, push_data, lk);
    ...
    push_data["t"] = std::to_string(
        std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch())
            .count());
    CPPUNIT_ASSERT(sendPush(nodeClient, push_data, lk) == dht::PushNotificationResult::ValuesExpired);
...
DhtProxyTester::testPushNotification()
{
    ...
    push_data["t"] = std::to_string(
        std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch())
            .count());
    CPPUNIT_ASSERT(sendPush(nodeClient, push_data, lk) == dht::PushNotificationResult::Values);
...
DhtProxyTester::testPushMultiValueEditExpire()
{
    ...
    CPPUNIT_ASSERT(cv.wait_for(lk, 10s, [&] {
        return std::any_of(events.begin(), events.end(), [](const Event& e) { return e.id == 200 && e.expired; });
    }));
...
DhtProxyTester::testPushRefreshNoSpuriousExpire()
{
    ...
    CPPUNIT_ASSERT(cv.wait_for(lk, 10s, [&] {
        return std::any_of(events.begin(), events.end(), [](const Event& e) { return e.id == 20 && e.expired; });
    }));
...
DhtProxyTester::testProxyListenEditChain()
{
    ...
    // Verify: at least 2 adds, 0 expires during edits
    {
        ...
        for (const auto& e : events)
            if (!e.expired && e.seq > maxSeq)
                maxSeq = e.seq;
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Latest add should be the final version", (uint16_t) NUM_EDITS, maxSeq);
    }
    ...
    // Only 1 expire, with the latest seq
    {
        ...
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Expired value should be the latest", (uint16_t) NUM_EDITS, expiredSeq);
    }
