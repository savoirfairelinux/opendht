...
ValueCacheTester::testUpdateExpiration()
{
    ...
    // Update v1 -> v2: only the new value is emitted as an add,
    // no expire callback for the old value.
    cache.onValues({v2}, {}, {}, types, now);
    CPPUNIT_ASSERT_EQUAL(2, addCount);
    CPPUNIT_ASSERT_EQUAL_MESSAGE("No expire callback on edit", 0, expireCount);
}
