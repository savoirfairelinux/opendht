...
DhtRunnerTester::testListenEditChainThenExpire()
{
    ...
    int totalAdds;
    {
        ...
        for (const auto& e : events)
            if (!e.expired && e.seq > maxSeq)
                maxSeq = e.seq;
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Latest add should have the final seq", (uint16_t) NUM_EDITS, maxSeq);
    }
    ...
    // Verify exactly 1 expire with the latest seq
    {
        ...
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Expired value should be the latest version", (uint16_t) NUM_EDITS, expiredSeq);
    }
...
DhtRunnerTester::testListenMultiValuePartialExpire()
{
    ...
    // Wait for short-lived value to expire
    {
        ...
        CPPUNIT_ASSERT_MESSAGE("Short-lived value should expire", cv.wait_for(lk, 60s, [&] {
            return std::any_of(events.begin(), events.end(), [](const Event& e) { return e.id == 1 && e.expired; });
        }));
    }
