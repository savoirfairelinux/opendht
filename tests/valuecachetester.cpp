...
ValueCacheTester::testUpdateExpiration() {
    ...
    int expireCount = 0;
    dht::ValueCache cache([&](const std::vector<std::shared_ptr<dht::Value>>& vals, bool expired){
        if (expired) expireCount += vals.size();
        else addCount += vals.size();
    });
...
ValueCacheTester::testClear() {
    ...
    auto v1 = std::make_shared<dht::Value>(); v1->id = 1;
    auto v2 = std::make_shared<dht::Value>(); v2->id = 2;
