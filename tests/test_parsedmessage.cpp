...
// Provide symbols that parsed_message.h inline functions reference but
// are hidden in the shared library (compiled with -fvisibility=hidden).
namespace dht {
time_point
from_time_t(std::time_t t)
{
    auto dt = std::chrono::system_clock::from_time_t(t) - std::chrono::system_clock::now();
    return clock::now() + dt;
}
msgpack::object*
