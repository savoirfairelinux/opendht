...
cdef class DhtRunner(_WithID):
    ...
    def shutdown(self, shutdown_cb=None):
        ...
        self.thisptr.get().shutdown(cpp.Dht.bindShutdownCb(shutdown_callback, <void*>cb_obj))
    def isRunning(self):
    ...
    def get(self, InfoHash key, get_cb=None, done_cb=None):
        ...
        if get_cb:
            ...
            ref.Py_INCREF(cb_obj)
            self.thisptr.get().get(key._infohash, cpp.Dht.bindGetCb(get_callback, <void*>cb_obj), cpp.Dht.bindDoneCb(done_callback, <void*>cb_obj))
        else:
    ...
    def put(self, InfoHash key, Value val, done_cb=None):
        ...
        cb_obj = {'done':done_cb}
        ref.Py_INCREF(cb_obj)
        self.thisptr.get().put(key._infohash, val._value, cpp.Dht.bindDoneCb(done_callback, <void*>cb_obj))
    def listen(self, InfoHash key, get_cb):
        ...
        ref.Py_INCREF(cb_obj)
        t._t = self.thisptr.get().listen(t._h, cpp.Dht.bindGetCb(get_callback, <void*>cb_obj)).share()
        return t
