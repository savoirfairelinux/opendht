...
Bucket::randomNode(std::mt19937_64& rd)
{
    if (nodes.empty())
        return nullptr;
    unsigned expired_node_count = std::count_if(nodes.cbegin(),
                                                nodes.cend(),
                                                [](const decltype(nodes)::value_type& node) {
                                                    return node->isExpired();
                                                });
    auto prioritize_not_expired = expired_node_count < nodes.size();

    std::uniform_int_distribution<unsigned> rand_node(0,
                                                      prioritize_not_expired ? nodes.size() - expired_node_count - 1
                                                                             : nodes.size() - 1);
...
void
Bucket::sendCachedPing(net::NetworkEngine& ne)
{
    if (not cached)
        return;
...
bool
RoutingTable::onNewNode(
    const Sp<Node>& node, int confirm, const time_point& now, const InfoHash& myid, net::NetworkEngine& ne)
{
