...
int
NetworkEngine::send(const Blob& msg, int /*flags*/, const Node& node)
{
    DHT_LOG.d(node.id, "[node %s] send %lu", node.getId().to_c_str(), msg.size());
    uint8_t* data;
    size_t size;
