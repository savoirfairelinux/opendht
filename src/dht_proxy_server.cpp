...
DhtProxyServer::ServerStats::toJson() const
{
    ...
    result["serverStartTime"] = static_cast<Json::LargestInt>(std::chrono::system_clock::to_time_t(serverStartTime));
    result["lastUpdated"] = static_cast<Json::LargestInt>(std::chrono::system_clock::to_time_t(lastUpdated));
...
public:
    ConnectionListener() {};
    explicit ConnectionListener(std::function<void(restinio::connection_id_t)> onClosed)
...
DhtProxyServer::PermanentPut::msgpack_unpack(const msgpack::object& o)
{
    ...
    if (auto exp = findMapValue(o, "exp"sv)) {
        expiration = std::chrono::system_clock::from_time_t(exp->as<time_t>());
    }
...
DhtProxyServer::DhtProxyServer(const std::shared_ptr<DhtRunner>& dht,
    ...
    , dht_(dht)
    , persistPath_(config.persistStatePath)
    , logger_(logger)
    , printStatsTimer_(std::make_unique<asio::steady_timer>(*ioContext_, 3s))
    , serverStartTime_(clock::now())
    , connListener_(std::make_shared<ConnectionListener>(
          std::bind(&DhtProxyServer::onConnectionClosed, this, std::placeholders::_1)))
    ...
    , bundleId_(config.bundleId)
{
    ...
    if (config.identity.first and config.identity.second) {
        ...
        httpsServer_ = std::make_unique<restinio::http_server_t<RestRouterTraitsTls>>(
            ioContext_, std::forward<restinio::run_on_this_thread_settings_t<RestRouterTraitsTls>>(std::move(settings)));
        // run https server
        serverThread_ = std::thread([this] {
            httpsServer_->open_async([] { /*ok*/ }, [](std::exception_ptr ex) { std::rethrow_exception(ex); });
        ...
        httpServer_ = std::make_unique<restinio::http_server_t<RestRouterTraits>>(
            ioContext_, std::forward<restinio::run_on_this_thread_settings_t<RestRouterTraits>>(std::move(settings)));
        // run http server
        serverThread_ = std::thread([this]() {
                ...
                for (auto& put : puts_) {
                    for (auto& pput : put.second.puts) {
                        ...
                        if (not pput.second.pushToken.empty()) {
                            auto jsonProvider = [infoHash = put.first.toString(),
                                                 clientId = pput.second.clientId,
                                                 vid = pput.first,
                                                 sessionCtx = pput.second.sessionCtx]() {
                                Json::Value json;
                            ...
                            pput.second.expireNotifyTimer->async_wait(
                                std::bind(&DhtProxyServer::handleNotifyPushListenExpire,
                                          this,
                                          std::placeholders::_1,
                                          pput.second.pushToken,
                                          std::move(jsonProvider),
                                          pput.second.type,
                                          pput.second.topic));
                        }
#endif
                ...
                for (auto& pushListener : pushListeners_) {
                    for (auto& listeners : pushListener.second.listeners) {
                        for (auto& listener : listeners.second) {
                            // start listening
                            listener.internalToken
                                = dht_->listen(listeners.first,
                                               [this,
                                                infoHash = listeners.first,
                                                pushToken = pushListener.first,
                                                type = listener.type,
                                                clientId = listener.clientId,
                                                sessionCtx = listener.sessionCtx,
                                                topic = listener.topic](const std::vector<Sp<Value>>& values,
                                                                        bool expired) {
                                                   return this->handlePushListen(infoHash,
                                                                                 pushToken,
                                                                                 type,
                                                                                 clientId,
                                                                                 sessionCtx,
...
HttpResponse
DhtProxyServer::initHttpResponse(HttpResponse response)
{
...
DhtProxyServer::createRestRouter()
{
    ...
    // key.options
    router->add_handler(restinio::http_method_options(), "/:hash", std::bind(&DhtProxyServer::options, this, _1, _2));
    // key.get
    ...
    router->add_handler(restinio::http_method_subscribe(),
                        "/:hash",
                        std::bind(&DhtProxyServer::subscribe, this, _1, _2));
    // key.unsubscribe
    router->add_handler(restinio::http_method_unsubscribe(),
                        "/:hash",
                        std::bind(&DhtProxyServer::unsubscribe, this, _1, _2));
...
RequestStatus
DhtProxyServer::getNodeInfo(restinio::request_handle_t request, restinio::router::route_params_t /*params*/) const
{
    try {
...
RequestStatus
DhtProxyServer::getStats(restinio::request_handle_t request, restinio::router::route_params_t /*params*/)
{
    requestNum_++;
    try {
...
RequestStatus
DhtProxyServer::get(restinio::request_handle_t request, restinio::router::route_params_t params)
{
    requestNum_++;
    try {
        ...
        dht_->get(
            infoHash,
            [this, response](const std::vector<Sp<Value>>& values) {
            ...
            [response](bool /*ok*/) { response->done(); });
...
RequestStatus
DhtProxyServer::listen(restinio::request_handle_t request, restinio::router::route_params_t params)
{
    requestNum_++;
    ...
    try {
        ...
        session.token = dht_->listen(infoHash, [this, response](const std::vector<Sp<Value>>& values, bool expired) {
            for (const auto& value : values) {
...
RequestStatus
DhtProxyServer::pingPush(restinio::request_handle_t request, restinio::router::route_params_t /*params*/)
{
    requestNum_++;
    try {
        std::string err;
...
RequestStatus
DhtProxyServer::subscribe(restinio::request_handle_t request, restinio::router::route_params_t params)
{
    requestNum_++;
    try {
        InfoHash infoHash(params["hash"]);
        ...
        // Expiration
        auto timeout = clock::now() + proxy::OP_TIMEOUT;
        ...
        if (not newListener) {
            ...
            listener.internalToken = dht_->listen(infoHash,
                                                  [this,
                                                   ...
                                                   clientId,
                                                   sessionCtx = listener.sessionCtx,
                                                   topic](const std::vector<Sp<Value>>& values, bool expired) {
                                                      return this->handlePushListen(infoHash,
                                                                                    pushToken,
                                                                                    type,
                                                                                    clientId,
...
RequestStatus
DhtProxyServer::unsubscribe(restinio::request_handle_t request, restinio::router::route_params_t params)
{
    requestNum_++;
...
void
DhtProxyServer::handleNotifyPushListenExpire(const asio::error_code& ec,
                                             const std::string pushToken,
                                             std::function<Json::Value()> jsonProvider,
                                             PushType type,
...
void
DhtProxyServer::handleCancelPushListen(const asio::error_code& ec,
                                       const std::string pushToken,
                                       const InfoHash key,
    ...
    const std::string& token, Json::Value&& json, PushType type, bool highPriority, const std::string& topic)
{
    ...
    try {
        ...
        if (type == PushType::UnifiedPush) {
            ...
            notifications[0] = std::move(notification);
...
RequestStatus
DhtProxyServer::put(restinio::request_handle_t request, restinio::router::route_params_t params)
{
    requestNum_++;
    InfoHash infoHash(params["hash"]);
    if (!infoHash)
            ...
            if (permanent) {
                ...
                if (value->id == Value::INVALID_ID) {
                    for (auto& pp : sPuts.puts) {
                        if (pp.second.pushToken == pushToken and pp.second.clientId == clientId
                            and pp.second.value->contentEquals(*value)) {
                            pp.second.expireTimer->expires_at(timeout);
                            pp.second.expireTimer->async_wait(std::bind(&DhtProxyServer::handleCancelPermamentPut,
                                                                        this,
                                                                        std::placeholders::_1,
                                                                        infoHash,
                                                                        pp.second.value->id));
                ...
                pput.expireTimer->async_wait(
                    std::bind(&DhtProxyServer::handleCancelPermamentPut, this, std::placeholders::_1, infoHash, vid));
                ...
                if (pput.sessionCtx) {
                    ...
                    pput.expireNotifyTimer->async_wait(std::bind(&DhtProxyServer::handleNotifyPushListenExpire,
                                                                 this,
                                                                 std::placeholders::_1,
                                                                 pushToken,
                                                                 std::move(jsonProvider),
...
RequestStatus
DhtProxyServer::putSigned(restinio::request_handle_t request, restinio::router::route_params_t params) const
{
    requestNum_++;
    InfoHash infoHash(params["hash"]);
    if (!infoHash)
...
RequestStatus
DhtProxyServer::putEncrypted(restinio::request_handle_t request, restinio::router::route_params_t params)
{
    requestNum_++;
    InfoHash infoHash(params["hash"]);
    if (!infoHash)
...
RequestStatus
DhtProxyServer::options(restinio::request_handle_t request, restinio::router::route_params_t /*params*/)
{
    requestNum_++;
#ifdef OPENDHT_PROXY_SERVER_IDENTITY
...
RequestStatus
DhtProxyServer::getFiltered(restinio::request_handle_t request, restinio::router::route_params_t params)
{
    requestNum_++;
