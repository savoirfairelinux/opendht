...
print_usage()
{
    std::cout
        << "Usage: dhtnode [-v [-l logfile]] [-i] [-d] [-n network_id] [-p local_port] [-b bootstrap_host[:port]] "
           "[--proxyserver local_port] [--proxyserverssl local_port] [--bundleid bundleid] [--pushserver endpoint]"
        << std::endl
        << std::endl;
    print_info();
}
...
print_help()
{
    ...
    std::cout << std::endl
              << "Operations with the proxy:" << std::endl
    ...
    std::cout
        << std::endl
        << "Operations on the DHT:" << std::endl
    ...
    std::cout
        << std::endl
        << "Indexation operations on the DHT:" << std::endl
...
void
cmd_loop(std::shared_ptr<DhtRunner>& node,
         dht_params& params
#ifdef OPENDHT_PROXY_SERVER
         ,
    ...
    while (true) {
        ...
        if (op == "x" || op == "exit" || op == "quit") {
            ...
            node->getNodeInfo([&](const std::shared_ptr<dht::NodeInfo>& nodeInfo) {
                ...
                std::cout << "Storage has " << nodeInfo->storage_values << " values, using "
                          << (nodeInfo->storage_size / 1024) << " KB" << std::endl;
        ...
        if (false) {
            ...
            // Pht syntax
            iss >> index >> keystr;
            auto new_index = std::find_if(indexes.begin(),
                                          indexes.end(),
        ...
        if (op == "g") {
            ...
            node->get(
                id,
                [start, total](const std::vector<std::shared_ptr<Value>>& values) {
                    auto now = std::chrono::high_resolution_clock::now();
                    (*total) += values.size();
                    std::cout << "Get: found " << values.size() << " value(s) after " << print_duration(now - start)
                ...
                [start, total](bool ok) {
                    ...
                    std::cout << "Get: " << (ok ? "completed" : "failure") << ", took " << print_duration(end - start)
                              << " (total " << *total << ')' << std::endl;
                },
                {},
                dht::Where {rem});
            ...
            node->query(
                id,
                [start](const std::vector<std::shared_ptr<FieldValueIndex>>& field_value_indexes) {
                    auto now = std::chrono::high_resolution_clock::now();
                    for (auto& index : field_value_indexes) {
                        std::cout << "Query: found field value index after " << print_duration(now - start)
                                  << std::endl;
                ...
                [start](bool ok) {
                    auto end = std::chrono::high_resolution_clock::now();
                    std::cout << "Query: " << (ok ? "completed" : "failure") << ", took " << print_duration(end - start)
                              << std::endl;
                },
                dht::Query {rem});
        } else if (op == "l") {
            ...
            std::getline(iss, rem);
            auto total = std::make_shared<size_t>();
            auto token = node->listen(
                id,
                [total](const std::vector<std::shared_ptr<Value>>& values, bool expired) {
                    if (expired)
                        (*total) -= values.size();
                    else
                        (*total) += values.size();
                    std::cout << "Listen: found " << values.size() << " values" << (expired ? " expired" : "")
                              << " (total " << *total << ')' << std::endl;
                    for (const auto& value : values)
                        std::cout << "\t" << *value << std::endl;
                    return true;
        ...
        if (op == "cl") {
            ...
            node->put(id, value, [start, value](bool ok) {
                ...
                std::cout << "Put: " << (ok ? "success" : "failure") << ", took " << print_duration(end - start)
                          << ". Value ID: " << std::hex << value->id << std::endl;
            ...
            node->putSigned(id, std::move(value), [start](bool ok) {
                auto end = std::chrono::high_resolution_clock::now();
                std::cout << "Put signed: " << (ok ? "success" : "failure") << " (took " << print_duration(end - start)
                          << "s)" << std::endl;
            });
        } else if (op == "e") {
            if (not params.id.first) {
            ...
            iss >> port;
            node->put(id,
                      dht::Value {dht::IpServiceAnnouncement::TYPE.id, dht::IpServiceAnnouncement(port)},
                      [start](bool ok) {
                          auto end = std::chrono::high_resolution_clock::now();
                          std::cout << "Announce: " << (ok ? "success" : "failure") << " (took "
                                    << print_duration(end - start) << std::endl;
                      });
        }
#ifdef OPENDHT_INDEXATION
                ...
                indexes.at(index).lookup(
                    ...
                    [=](std::vector<std::shared_ptr<indexation::Value>>& vals, indexation::Prefix p) {
                        ...
                        for (const auto& v : vals)
                            std::cout << "      " << v->first.toString() << "[vid: " << std::hex << v->second
                                      << std::dec << "]" << std::endl;
                    },
                    [start](bool ok) {
                        auto end = std::chrono::high_resolution_clock::now();
                        std::cout << "Pht::lookup: " << (ok ? "done." : "failed.") << " took "
                                  << print_duration(end - start) << std::endl;
                    },
            ...
            try {
                auto key = createPhtKey(parseStringMap(keystr));
                indexes.at(index).insert(key, v, [=](bool ok) {
