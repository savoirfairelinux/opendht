...
extern "C" fn get_handler_cb(v: *mut Value, ptr: *mut c_void) -> bool {
    if ptr.is_null() {
        return true;
    }
    unsafe {
...
extern "C" fn done_handler_cb(ok: bool, ptr: *mut c_void) {
    unsafe {
...
extern "C" fn put_handler_done(ok: bool, ptr: *mut c_void) {
    unsafe {
...
extern "C" fn listen_handler(v: *mut Value, expired: bool, ptr: *mut c_void) -> bool {
    unsafe {
...
extern "C" fn listen_handler_done(ptr: *mut c_void) {
    unsafe {
...
impl DhtRunner {
    pub fn new() -> Box<DhtRunner> {
        unsafe { Box::from_raw(dht_runner_new()) }
    }

    pub fn run(&mut self, port: u16) {
        unsafe { dht_runner_run(&mut *self, port) }
    }

    pub fn run_config(&mut self, port: u16, config: Box<DhtRunnerConfig>) {
        unsafe { dht_runner_run_config(&mut *self, port, &*config) }
    }
    ...
    pub fn node_id(&self) -> InfoHash {
        unsafe { dht_runner_get_node_id(&*self) }
    }

    pub fn id(&self) -> InfoHash {
        unsafe { dht_runner_get_id(&*self) }
    }
    ...
    pub fn get<'a>(
        ...
        let handler = Box::new(GetHandler { get_cb, done_cb });
        let handler = Box::into_raw(handler) as *mut c_void;
        unsafe { dht_runner_get(&mut *self, h, get_handler_cb, done_handler_cb, handler) }
    }
    ...
    pub fn put<'a>(
        ...
        let handler = Box::new(PutHandler { done_cb });
        let handler = Box::into_raw(handler) as *mut c_void;
        unsafe { dht_runner_put(&mut *self, h, &*v, put_handler_done, handler, permanent) }
    }
    ...
    pub fn put_signed<'a>(
        ...
        let handler = Box::new(PutHandler { done_cb });
        let handler = Box::into_raw(handler) as *mut c_void;
        unsafe { dht_runner_put_signed(&mut *self, h, &*v, put_handler_done, handler, permanent) }
    }
    ...
    pub fn cancel_put<'a>(&mut self, h: &InfoHash, vid: u64) {
        unsafe { dht_runner_cancel_put(&mut *self, h, vid) }
    }
    ...
    pub fn listen<'a>(
        ...
        let handler = Box::new(ListenHandler { cb });
        let handler = Box::into_raw(handler) as *mut c_void;
        unsafe {
            Box::from_raw(dht_runner_listen(
                &mut *self,
                h,
                listen_handler,
                listen_handler_done,
    ...
    pub fn cancel_listen(&mut self, h: &InfoHash, token: Box<OpToken>) {
        unsafe { dht_runner_cancel_listen(&mut *self, h, &*token) }
    }

    pub fn shutdown(
        &mut self,
        done_cb: extern "C" fn(bool, *mut c_void),
        cb_user_data: *mut c_void,
    ) {
        unsafe { dht_runner_shutdown(&mut *self, done_cb, cb_user_data) }
    }
    ...
    pub fn public_addresses(&self) -> Vec<SocketAddr> {
        ...
        unsafe {
            ...
            while !addresses.is_null() && !(*addresses).is_null() {
                ...
                addresses = (addresses as usize + std::mem::size_of::<*mut OsSocketAddr>())
                    as *mut *mut OsSocketAddr;
            }
        }
        result
    }
...
impl Drop for DhtRunner {
    fn drop(&mut self) {
        unsafe { dht_runner_delete(&mut *self) }
    }
}

impl Drop for OpToken {
    fn drop(&mut self) {
        unsafe { dht_op_token_delete(&mut *self) }
    }
}
